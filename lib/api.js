...
//     bulkDelete: (ids) => api.post("/accounts/users/bulk-delete/", { ids }),
//     bulkUpdateStatus: (ids, is_active) =>
//       api.post("/accounts/users/bulk-status/", { ids, is_active }),
//     export: (fmt = "xlsx") =>
//       downloadFile(`/accounts/staff/export/?format=${fmt}`, `staff_users.${fmt}`),
//     template: (fmt = "xlsx") =>
//       downloadFile(
//         `/accounts/staff/export/?format=${fmt}&template=true`,
//         `staff_template.${fmt}`,
//       ),
//     import: (file) => uploadFile("/accounts/staff/import/", file),
//   },
// };
...
//     exportAttendance: (sessionId, fmt = "pdf", section = "") => {
//       const q = section ? `?section=${section}` : "";
//       return downloadFile(
//         `/exam-sessions/${sessionId}/export/${q}`,
//         `attendance.${fmt}`,
//       );
//     },
...
export const coreApi = {
  ...
  students: {
    ...
    delete: (id) => api.delete(`/students/${id}/`),
    bulk: (data) => api.post("/students/bulk/", data),
    // Bulk activate/deactivate/delete for the selection checkboxes on the
    // students list. action: "activate" | "deactivate" | "delete"
    bulkAction: (action, ids) =>
      api.post("/students/bulk-action/", { action, ids }),
    export: (fmt = "xlsx") =>
      downloadFile(`/students/export/`, `students.${fmt}`),
    template: (fmt = "xlsx") =>
      downloadFile(
