...
import { Plus, Edit2, Trash2, X, Loader2, Phone, ShieldCheck, ScanLine } from "lucide-react";
...
function UserModal({ user, onClose }) {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      ...
      toast.error(typeof d === "string" ? d : (d?.detail ?? Object.values(d ?? {})?.[0]?.[0] ?? "Error"));
    },
  });
          ...
          {!user && (
            <>
              <div className="space-y-1.5">
                ...
                <input type="password" value={form.password} onChange={(e) => upd("password", e.target.value)} required className={inputCls} />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs text-white/40">Confirm Password *</label>
                <input type="password" value={form.confirm_password} onChange={(e) => upd("confirm_password", e.target.value)} required className={inputCls} />
              </div>
            </>
          )}
...
export default function UsersPage() {
  ...
  const handleDelete = (u) => {
    if (confirm(`Delete account for ${u.username}?`)) deleteMutation.mutate(u.id);
  };
