...
export default function SessionsPage() {
  ...
  const handleExport = async (fmt) => {
    setExporting(true);
    try { await coreApi.sessions.export(fmt); }
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Create */}
          {isAdmin && (
            <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 px-4 h-9 rounded-xl bg-teal-500 hover:bg-teal-400 text-navy-950 font-semibold text-sm transition-all shadow-[0_0_20px_rgba(45,212,191,0.2)]">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48 max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search course or venue…" className="w-full h-10 pl-9 pr-4 rounded-xl bg-navy-800 border border-white/10 text-white placeholder-white/25 text-sm focus:outline-none focus:border-teal-500/40" />
        </div>
        <div className="flex gap-1 bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
          {STATUS_FILTERS.map((f) => (
            <button key={f} onClick={() => setStatusFilter(f)} className={cn("px-3 h-8 rounded-lg text-xs font-medium capitalize transition-all", statusFilter === f ? "bg-teal-500/10 text-teal-400 border border-teal-500/20" : "text-white/40 hover:text-white/70")}>{f}</button>
          ))}
        </div>
      </div>
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        <p className="text-white/35 text-xs mt-0.5">{formatTime(s.start_time)}{s.end_time ? ` – ${formatTime(s.end_time)}` : ""}</p>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          {isAdmin && s.status === "scheduled" && (
                            <button onClick={() => statusMutation.mutate({ id: s.id, s: "active" })} disabled={statusMutation.isPending} title="Activate" className="p-1.5 rounded-lg bg-teal-500/10 text-teal-400 hover:bg-teal-500/20 transition-colors"><Play className="w-3.5 h-3.5" /></button>
                          )}
      ...
      {showImport && (
        <ImportModal
          ...
          description="Upload a CSV or XLSX file. Each row creates a new scheduled session."
          templateHint={["course_code", "programme_code", "level_name", "date", "start_time", "end_time", "venue", "expected_students"]}
