...
import { Users, BookOpen, Activity, Scan, ArrowRight, Clock, MapPin } from "lucide-react";
import Link from "next/link";
import { formatDate, formatTime, getGreeting, attendancePct } from "@/lib/utils";
...
function SessionCard({ session }) {
  const pct = attendancePct(session.attendance_summary?.present, session.expected_students);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <p className="font-semibold text-white text-sm">{session.course_code}</p>
          <p className="text-white/40 text-xs mt-0.5 line-clamp-1">{session.course_title}</p>
        </div>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-xs">
          <span className="text-white/40">
            {session.attendance_summary?.present ?? 0} / {session.expected_students} present
          </span>
...
export default function DashboardPage() {
  ...
  const { data: activeSessData } = useQuery({
    ...
    queryFn:         () => coreApi.sessions.list({ status: "active", page_size: 10 }).then((r) => r.data),
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 max-w-7xl">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard label="Total Students"  value={stats?.total_students}    icon={Users}    colorClass="text-teal-400 bg-teal-500/10 border-teal-500/20"   delay={0}   />
        <StatCard label="Exam Sessions"   value={stats?.total_sessions}    icon={BookOpen} colorClass="text-sky-400 bg-sky-500/10 border-sky-500/20"       delay={50}  />
        <StatCard label="Active Now"      value={stats?.active_sessions}   icon={Activity} colorClass="text-amber-400 bg-amber-500/10 border-amber-500/20" delay={100} />
        <StatCard label="Scans Today"     value={stats?.total_scans_today} icon={Scan}     colorClass="text-violet-400 bg-violet-500/10 border-violet-500/20" delay={150} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/sessions?status=active" className="text-xs text-teal-400 hover:text-teal-300 flex items-center gap-1">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/sessions" className="text-xs text-teal-400 hover:text-teal-300 flex items-center gap-1">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {recentData?.results?.length === 0 && (
                  <tr>
                    <td colSpan={4} className="text-center text-white/25 py-8 text-sm">No sessions yet</td>
                  </tr>
                )}
                {recentData?.results?.map((s) => (
                  <tr key={s.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/sessions/${s.id}/attendance`} className="font-mono text-teal-400 hover:text-teal-300 text-xs">
                        {s.course_code}
                      </Link>
                      <p className="text-white/30 text-xs truncate max-w-[130px]">{s.course_title}</p>
                    </td>
                    <td className="px-4 py-3 text-xs text-white/50">{formatDate(s.date)}</td>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/40 text-sm">Start scanning student QR codes</p>
            </div>
          </div>
